(() => {
  ...
  // First-visit boot. Content is never hidden; overlay removes itself independently at 3.2s.
  const boot=$("#exmBoot"),skip=$("#bootSkip"),sound=$("#soundToggle");
  if(boot){
    let released=false,ctx=null,soundOn=false;
    const release=()=>{if(released)return;released=true;boot.classList.add("boot-release");setTimeout(()=>{boot.remove();try{localStorage.setItem("exmBootComplete","1");}catch(e){}},reducedMotion?120:420);};
    let seen=false;try{seen=localStorage.getItem("exmBootComplete")==="1";}catch(e){}
    if(seen){boot.remove();}else{
      document.documentElement.classList.add("boot-active");
      setTimeout(()=>skip?.classList.add("visible"),1000);
      const normal=setTimeout(release,reducedMotion?500:2800);
      const emergency=setTimeout(release,3200);
      skip?.addEventListener("click",()=>{clearTimeout(normal);clearTimeout(emergency);release();});
      sound?.addEventListener("click",()=>{
        soundOn=!soundOn;sound.setAttribute("aria-pressed",String(soundOn));sound.textContent=`SOUND: ${soundOn?"ON":"OFF"}`;
        if(soundOn){try{ctx=ctx||new(window.AudioContext||window.webkitAudioContext)();const o=ctx.createOscillator(),g=ctx.createGain();o.type="sine";o.frequency.value=92;g.gain.setValueAtTime(.025,ctx.currentTime);g.gain.exponentialRampToValueAtTime(.0001,ctx.currentTime+.09);o.connect(g);g.connect(ctx.destination);o.start();o.stop(ctx.currentTime+.1);}catch(e){}}
      });
    }
  }

  const toggle=$(".nav-toggle"),links=$(".nav-links"),more=$(".nav-more"),moreToggle=$(".nav-more-toggle");
  const closeMore=()=>{more?.classList.remove("open");moreToggle?.setAttribute("aria-expanded","false");};
  const closeNav=()=>{links?.classList.remove("open");toggle?.setAttribute("aria-expanded","false");closeMore();};
  if(toggle&&links){toggle.addEventListener("click",e=>{e.stopPropagation();const open=links.classList.toggle("open");toggle.setAttribute("aria-expanded",String(open));if(!open)closeMore();});links.addEventListener("click",e=>{if(e.target.closest("a"))closeNav();});}
  if(more&&moreToggle){moreToggle.addEventListener("click",e=>{e.stopPropagation();const open=more.classList.toggle("open");moreToggle.setAttribute("aria-expanded",String(open));});}
  document.addEventListener("click",e=>{if(links&&!links.contains(e.target)&&e.target!==toggle)closeNav();});
  document.addEventListener("keydown",e=>{if(e.key==="Escape"){closeNav();toggle?.focus();}});

  const page=(location.pathname.split("/").pop()||"index.html").toLowerCase();
  $$(".nav-links a[data-page]").forEach(a=>{if(a.dataset.page===page){a.classList.add("active");a.setAttribute("aria-current","page");if(a.closest(".nav-more"))moreToggle?.classList.add("active");}});
